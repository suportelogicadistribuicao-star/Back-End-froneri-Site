...
exports.devRouter = exports.tickRouter = exports.cadRouter = exports.rotRouter = exports.rupturaRouter = void 0;
// ─── rupturaRoutes.js ─────────────────────────────────────────────────────────
const express_1 = __importDefault(require("express"));
const database_1 = require("../config/database");
const auth_1 = require("../middleware/auth");
const rupturaRouter = express_1.default.Router();
exports.rupturaRouter = rupturaRouter;
rupturaRouter.get('/', auth_1.authMiddleware, auth_1.ownDataOnly, async (req, res) => {
    try {
        ...
        const total = await (0, database_1.query)(`SELECT COUNT(*) AS count FROM ruptura r ${whereSql}`, params);
...
const rotRouter = express_1.default.Router();
exports.rotRouter = rotRouter;
rotRouter.get('/', auth_1.authMiddleware, auth_1.ownDataOnly, async (req, res) => {
    try {
...
rotRouter.get('/exportar/:vendedorId', auth_1.authMiddleware, async (req, res) => {
    try {
        const { dia } = req.query;
        ...
        const csv = [
            ...
            ...rows.rows.map(r => `${r.sequencia};${r.dia_semana};${r.sold};"${r.razao_social}";` +
...
// ─── cadastrosRoutes.js ───────────────────────────────────────────────────────
const cadRouter = express_1.default.Router();
exports.cadRouter = cadRouter;
cadRouter.get('/', auth_1.authMiddleware, auth_1.ownDataOnly, async (req, res) => {
    try {
...
tickRouter.get('/', auth_1.authMiddleware, auth_1.ownDataOnly, async (req, res) => {
    try {
...
devRouter.get('/', auth_1.authMiddleware, auth_1.ownDataOnly, async (req, res) => {
    try {
