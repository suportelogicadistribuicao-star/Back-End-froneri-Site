...
router.post('/register', async (req, res) => {
    try {
        ...
        const totalRes = await query('SELECT COUNT(*)::int AS total FROM usuarios');
        ...
        const existente = await query('SELECT id FROM usuarios WHERE email = $1 LIMIT 1', [emailNormalizado]);
...
router.post('/register-vendedor', authMiddleware, requireRole('admin'), async (req, res) => {
    try {
        ...
        const emailEmUso = await query('SELECT id FROM usuarios WHERE email = $1 LIMIT 1', [emailNormalizado]);
...
router.put('/senha', authMiddleware, alterarSenhaHandler);

// PATCH /api/auth/trocar-senha
router.patch('/trocar-senha', authMiddleware, alterarSenhaHandler);

// PATCH /api/auth/reset-senha-vendedor
// Admin redefine a senha do usuario vinculado a um vendedor.
router.patch('/reset-senha-vendedor', authMiddleware, requireRole('admin'), async (req, res) => {
    try {
        ...
        await query('UPDATE usuarios SET senha_hash = $1 WHERE id = $2', [hash, vendedor.usuario_id]);
