...
async function importarBaseAtiva(filePath, usuarioId) {
    ...
    try {
        ...
        if (baseRows.length > 0) {
            ...
            await query('UPDATE roteirizacao SET ativa = FALSE WHERE ativa = TRUE');
            ...
            await withTransaction(async (client) => {
                for (const row of baseRows) {
                    try {
                        ...
                        const vendedorId = resolveVendedorId(
                            null,
                            norm(row['Description']),  // ex: Vendedor 01_Logica MG
                            null
                        );
        ...
        if (devedoresRows.length > 0) {
            ...
            await query('DELETE FROM devedores');
            await withTransaction(async (client) => {
        ...
        await withTransaction(async (client) => {
            for (const row of cadRows) {
                try {
                    ...
                    const nomeVendedor = norm(
                        row['VENDEDOR - TERRITÓRIO (que irá tirar pedido do cliente)']
                    );
