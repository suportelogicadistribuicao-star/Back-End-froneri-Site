...
// Diz se já existe alguma linha gravada por mes/ano na tabela informada —
// base de `hasSnapshotForPeriodo` e `hasRupturaForPeriodo` abaixo, que só
// diferem na tabela consultada.
async function existsForPeriodo(table: 'clientes_historico_mensal' | 'ruptura', mes: number, ano: number): Promise<boolean> {
    const r = await query(
        `SELECT 1 FROM ${table} WHERE mes_numero = $1 AND ano = $2 LIMIT 1`,
        [mes, ano]
    );
...
async function hasSnapshotForPeriodo(mes: number, ano: number): Promise<boolean> {
    return existsForPeriodo('clientes_historico_mensal', mes, ano);
}
...
async function hasRupturaForPeriodo(mes: number, ano: number): Promise<boolean> {
    return existsForPeriodo('ruptura', mes, ano);
}
